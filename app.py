...
def predict():
    ...
    if request.method == "POST":
        ...
        output = round(prediction * 100, 2)

    return render_template('index.html', prediction_text=("The document samples show  " + str(output) + " percent similarity."))

if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000)
    
## to run the app on windows give link "localhost:5000" on web browser.  
